...
public class Catmul : MonoBehaviour
{
    ...
    public void GenerateSpline()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            points[i] = new Vector3(x - 15 + i * 10, 0, z - 25); //bottom   (0123)
            points[i + 4] = new Vector3(x + 25, 0, z - 15 + i * 10);//right (4567)
            points[i + 8] = new Vector3(x + 15 - i * 10, 0, z + 25);//top   (891011)
            points[i + 12] = new Vector3(x - 25, 0, z + 15 - i * 10);//left (12131415)
        }
        pointIndex = 0;
    ...
    public void DrawLine()
    {
        ...
        for (int i = 0; i < spc; i++)
        {
            Vector3 linepoints = sp[i].transform.position;
            linepoints.y += 0.01f;
            line.SetPosition(i, linepoints);
        }
    }
}